...
namespace {
	...
	template<benchmark_types benchmark_type> struct final_test_results_accessor : public final_test_results<benchmark_type> {
		using base = final_test_results<benchmark_type>;
		using base::base;
		final_test_results_accessor(base b) : base(std::move(b)) {
		}
		using base::result_to_csv_line;
		using base::md_separator_row;
		using base::result_to_md_row;
		using base::stage_name_str;
		using base::md_header_row;
		using base::sorted_results;
		using base::csv_header;
		using base::test_name;
	};

	template<stage_config_data stage_config> struct stage_results_data_accessor : public stage_results_data<stage_config> {
		using base = stage_results_data<stage_config>;
		using base::base;
		stage_results_data_accessor(base b) : base(std::move(b)) {
		}
		using base::accum_to_csv_line;
		using base::stage_name_str;
		using base::lib_positions;
		using base::csv_header;
		using base::results;
	};
...
int32_t main() {
	...
	try {
		...
		check(unit_test<"tie_zero_var_diff_mean_not_tied", true, true>::run([] {
			auto a = make_bench(100.0, 0.0, 50, 0.0, true);
		...
		check(unit_test<"rank_empty_yields_empty", true, true>::run([] {
			test_data td{};
			td.test_name										  = "Empty";
			final_test_results_accessor<benchmark_types::cpu> res = process_test_rankings<"rank_empty_yields_empty", benchmark_types::cpu>(td);
		...
		check(unit_test<"rank_excludes_unconverged", true, true>::run([] {
			...
			final_test_results_accessor<benchmark_types::cpu> res = process_test_rankings<"rank_excludes_unconverged", benchmark_types::cpu>(td);
		...
		check(unit_test<"rank_excludes_unconverged_with_multiple_survivors", true, true>::run([] {
			...
			td.results["Ghost"]									  = make_bench(10.0, 0.01, 100, 2000.0, false);
			final_test_results_accessor<benchmark_types::cpu> res = process_test_rankings<"rank_excludes_unconverged_with_multiple_survivors", benchmark_types::cpu>(td);
			return res.sorted_results.empty();
		}));
		...
		check(unit_test<"rank_clear_winner_and_loser", true, true>::run([] {
			...
			final_test_results_accessor<benchmark_types::cpu> res = process_test_rankings<"rank_clear_winner_and_loser", benchmark_types::cpu>(td);
		...
		check(unit_test<"rank_statistical_tie_shares_rank", true, true>::run([] {
			...
			final_test_results_accessor<benchmark_types::cpu> res = process_test_rankings<"rank_statistical_tie_shares_rank", benchmark_types::cpu>(td);
		...
		check(unit_test<"rank_top_throughput_sorted_first", true, true>::run([] {
			...
			final_test_results_accessor<benchmark_types::cpu> res = process_test_rankings<"rank_top_throughput_sorted_first", benchmark_types::cpu>(td);
		...
		check(unit_test<"stage_aggregates_wins_and_losses", true, true>::run([] {
			...
			stage_data sd{};
			sd.results.push_back(t1);
			sd.results.push_back(t2);
			stage_results_data_accessor<stage_config_data{}> res = process_stage_rankings<"stage_aggregates_wins_and_losses", stage_config_data{}>(sd);
		...
		check(unit_test<"stage_sorted_by_wins_desc", true, true>::run([] {
			...
			stage_data sd{};
			sd.results.push_back(t1);
			stage_results_data_accessor<stage_config_data{}> res = process_stage_rankings<"stage_sorted_by_wins_desc", stage_config_data{}>(sd);
		...
		check(unit_test<"stage_counts_ties", true, true>::run([] {
			...
			stage_data sd{};
			sd.results.push_back(t1);
			stage_results_data_accessor<stage_config_data{}> res = process_stage_rankings<"stage_counts_ties", stage_config_data{}>(sd);
		...
		check(unit_test<"fmt_csv_header_fields", true, true>::run([] {
			auto h = final_test_results_accessor<benchmark_types::cpu>::csv_header(false);
		...
		check(unit_test<"fmt_md_header_starts_pipe", true, true>::run([] {
			return final_test_results_accessor<benchmark_types::cpu>::md_header_row(false).rfind("| Library", 0) == 0;
		}));

		check(unit_test<"fmt_md_separator_has_dashes", true, true>::run([] {
			return contains(final_test_results_accessor<benchmark_types::cpu>::md_separator_row(false), "---");
		}));

		check(unit_test<"fmt_csv_line_win_label", true, true>::run([] {
			auto r	  = make_completion("LibWin", 123.0, position_type::win, 1);
			auto line = final_test_results_accessor<benchmark_types::cpu>::result_to_csv_line(r, false);
		...
		check(unit_test<"fmt_md_row_marks_tie", true, true>::run([] {
			auto r	 = make_completion("LibTie", 99.0, position_type::tie, 1);
			auto row = final_test_results_accessor<benchmark_types::cpu>::result_to_md_row(r, false);
			return contains(row, "STATISTICAL TIE");
		}));

		check(unit_test<"fmt_to_csv_emits_rows", true, true>::run([] {
			final_test_results_accessor<benchmark_types::cpu> ftr{ "fmt_to_csv_emits_rows" };
			ftr.test_name = "Demo";
		...
		check(unit_test<"fmt_to_markdown_emits_rows", true, true>::run([] {
			final_test_results_accessor<benchmark_types::cpu> ftr{ "fmt_to_markdown_emits_rows" };
			ftr.test_name = "Demo";
		...
		check(unit_test<"fmt_stage_csv_header_fields", true, true>::run([] {
			auto h = stage_results_data_accessor<stage_config_data{}>::csv_header();
			return contains(h, "Wins") && contains(h, "Ties") && contains(h, "Losses");
		}));

		check(unit_test<"fmt_stage_accum_line", true, true>::run([] {
			stage_results_data_accessor<stage_config_data{}> srd{ "fmt_stage_accum_line" };
		...
		check(unit_test<"fmt_stage_to_csv", true, true>::run([] {
			stage_results_data_accessor<stage_config_data{}> srd{ "fmt_stage_to_csv" };
