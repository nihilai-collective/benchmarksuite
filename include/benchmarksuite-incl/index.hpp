...
namespace benchmarksuite {
	...
	template<benchmark_types benchmark_type> struct final_test_results {
	  ...
	  protected:
		...
		BNCH_SWT_HOST static void write_optional_cell(std::ostream& ss, const char* separator, bool include, const std::optional<value_type>& value) {
			if (include) {
				ss << separator;
				if (value.has_value()) {
					ss << value.value();
	...
	template<string_literal stage_name_new, const stage_config_data stage_config> struct benchmark_stage {
		...
		BNCH_SWT_HOST static auto get_test_results(const std::string& test_name) {
			auto& results = get_raw_test_data().results;
			auto iter	  = results.find(test_name);
			if (iter == results.end()) {
				return final_test_results<stage_config.benchmark_type>{ stage_name.operator std::string_view() };
			}
			return process_test_rankings<stage_name_new, stage_config.benchmark_type>(iter->second);
		}
